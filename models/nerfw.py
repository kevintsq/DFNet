...
class Embedder:
    ...
    def create_embedding_fn(self):
        ...
        if self.kwargs['log_sampling']:
            freq_bands = 2. ** torch.linspace(0., max_freq, steps=self.N_freqs,
                                              device="cuda")  # tensor([  1.,   2.,   4.,   8.,  16.,  32.,  64., 128., 256., 512.])
        else:
    ...
    def get_embed_weight(self, epoch, num_freqs, N):
        ...
        for i in range(num_freqs):
            tmp = torch.clamp(alpha - i, 0, 1)
            tmp2 = (1 - torch.cos(torch.pi * tmp)) / 2
            W_j.append(tmp2)
        return W_j
...
def create_nerf(args):
    ...
    if args.reduce_embedding == 2:  # use DNeRF embedding
        embed_fn, input_ch, embedder_obj = get_embedder(args.multires, args.i_embed, args.reduce_embedding,
                                                        args.epochToMaxFreq)  # input_ch.shape=63
    else:
    ...
    if args.use_viewdirs:
        if args.reduce_embedding == 2:  # use DNeRF embedding
            if args.no_DNeRF_viewdir:  # no DNeRF embedding for viewdir
                ...
                embeddirs_fn, input_ch_views, embedddirs_obj = get_embedder(args.multires_views, args.i_embed,
                                                                            args.reduce_embedding, args.epochToMaxFreq)
        else:
            embeddirs_fn, input_ch_views, _ = get_embedder(args.multires_views, args.i_embed,
                                                           args.reduce_embedding)  # input_ch_views.shape=27
    ...
    if args.NeRFH:
        model = NeRFW('coarse', D=args.netdepth, W=args.netwidth, skips=skips, in_channels_xyz=input_ch,
    ...
    if args.NeRFH:
        network_query_fn = lambda inputs, viewdirs, ts, network_fn, \
                                  typ, embedding_a, embedding_t, output_transient, test_time: run_network_NeRFW(inputs,
                                                                                                                viewdirs,
                                                                                                                ts,
                                                                                                                network_fn,
                                                                                                                embed_fn=embed_fn,
