...
def sample_pdf(bins, weights, N_samples, det=False, pytest=False):
    ...
    if det:
        u = torch.linspace(0., 1., steps=N_samples, device="cuda")
...
def raw2outputs(raw, z_vals, rays_d, raw_noise_std=0, white_bkgd=False, pytest=False):
    ...
    dists = torch.cat([dists, torch.tensor([1e10]).expand(dists[..., :1].shape)],
                      -1)  # [N_rays, N_samples] the last delta is infinity (1e10)
    ...
    noise = 0.
    if raw_noise_std > 0.:
        noise = torch.randn(raw[..., 3].shape, device="cuda") * raw_noise_std

        # Overwrite randomly sampled data if pytest
        if pytest:
            np.random.seed(0)
            noise = torch.rand(*list(raw[..., 3].shape), device="cuda") * raw_noise_std
    ...
    weights = alpha * torch.cumprod(torch.cat([torch.ones((alpha.shape[0], 1), device="cuda"),
                                               1. - alpha + 1e-10], -1), -1)[:, :-1]
...
def raw2outputs_NeRFW(raw, z_vals, rays_d, raw_noise_std=0, output_transient=False, beta_min=0.1, white_bkgd=False,
                      test_time=False, static_only=True, typ="coarse"):
...
def render_rays(ray_batch,
    ...
    t_vals = torch.linspace(0., 1., steps=N_samples, device=ray_batch.device)  # [N_samples]
    if not lindisp:
    ...
    if perturb > 0.:
        ...
        t_rand = torch.rand(z_vals.shape, device=ray_batch.device)  # randomly choose in intervals

        z_vals = lower + (upper - lower) * t_rand

    pts = rays_o[..., None, :] + rays_d[..., None, :] * z_vals[..., :,
                                                        None]  # [N_rays, N_samples, 3] # Sample 3D point print it out, [batchsize, 64 samples, 3 axis]
    ...
    rgb_map, disp_map, acc_map, weights, depth_map, _, _ = raw2outputs_NeRFW(raw, z_vals, rays_d, raw_noise_std,
                                                                             white_bkgd, test_time=test_time,
                                                                             typ="coarse")
    if N_importance > 0:
        ...
        # inference fine MLPs
        if i_epoch >= 0:  # Coarse-to-fine
            raw = network_query_fn(pts, viewdirs, img_idxs, network_fine, 'fine', embedding_a, embedding_t,
                                   output_transient, test_time=test_time, epoch=i_epoch)
        else:
            raw = network_query_fn(pts, viewdirs, img_idxs, network_fine, 'fine', embedding_a, embedding_t,
                                   output_transient, test_time=test_time)

        rgb_map, disp_map, acc_map, weights, depth_map, transient_sigmas, beta = raw2outputs_NeRFW(raw, z_vals, rays_d,
                                                                                                   raw_noise_std,
                                                                                                   output_transient,
                                                                                                   network_fine.beta_min,
                                                                                                   white_bkgd,
                                                                                                   test_time,
                                                                                                   typ="fine")
...
def render(H, W, fx, fy, chunk=1024 * 32, rays=None, c2w=None, ndc=True,
           near=0., far=1.,
           use_viewdirs=False, c2w_staticcam=None, img_idx=0,
           **kwargs):
    if c2w is not None:
        # special case to render full image
        rays_o, rays_d = get_rays(H, W, fx, fy, c2w)
    else:
    ...
    if use_viewdirs:
        ...
        if c2w_staticcam is not None:
            ...
            rays_o, rays_d = get_rays(H, W, fx, fy, c2w_staticcam)
    ...
    if ndc:
        ...
        rays_o, rays_d = ndc_rays(H, W, fx, fy, 1., rays_o, rays_d)
...
def render_path(args, render_poses, hwf, chunk, render_kwargs, gt_imgs=None, savedir=None, render_factor=0,
                single_gt_img=False, img_ids=0):
    ...
    for i, c2w in enumerate(tqdm(render_poses)):
        t = time.time()
        rgb, disp, acc, extras = render(H, W, fx, fy, chunk=chunk, c2w=c2w[:3, :4], img_idx=img_ids[i], **render_kwargs)
...
def render_test(args, train_dl, val_dl, hwf, start, render_kwargs_test, decoder_coarse=None, decoder_fine=None):
    ...
    trainsavedir = os.path.join(args.basedir, args.expname,
                                'evaluate_train_{}_{:06d}'.format('test' if args.render_test else 'path', start))
    ...
    with torch.no_grad():
        ...
        rgbs, disps = render_path(args, poses_train.to(device), hwf, args.chunk, render_kwargs_test,
                                  gt_imgs=images_train, savedir=trainsavedir, img_ids=index_train)
    ...
    testsavedir = os.path.join(args.basedir, args.expname,
                               'evaluate_val_{}_{:06d}'.format('test' if args.render_test else 'path', start))
    ...
    with torch.no_grad():
        ...
        rgbs, disps = render_path(args, poses_val.to(device), hwf, args.chunk, render_kwargs_test, gt_imgs=images_val,
