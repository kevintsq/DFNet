...
def fix_coord(args, train_set, val_set, pose_avg_stats_file='', rescale_coord=False):
    ...
    all_poses = np.concatenate(
        [all_poses[:, 0:1, :], -all_poses[:, 1:2, :], -all_poses[:, 2:3, :], all_poses[:, 3:4, :]], 1)
...
def load_colmap_dataloader(args):
    ...
    val_dl = DataLoader(val_set, batch_size=args.val_batch_size, shuffle=False, num_workers=4)
    test_dl = DataLoader(val_set, batch_size=1, shuffle=False, num_workers=1)

    hwf = (train_set.H, train_set.W, train_set.fx, train_set.fy)

    return train_dl, val_dl, test_dl, hwf, bounds.min(), bounds.max()


def load_colmap_dataloader_NeRF(args):
    ...
    if args.render_video_train or args.render_test:
        train_shuffle = False
    train_dl = DataLoader(train_set, batch_size=1, shuffle=train_shuffle, num_workers=1)
    ...
    val_dl = DataLoader(val_set, batch_size=1, shuffle=False, num_workers=1)

    hwf = (train_set.H, train_set.W, train_set.fx, train_set.fy)

    return train_dl, val_dl, hwf, bounds, render_poses, render_img
...
def main():
    ...
    # python load_colmap.py --config ../script/config/7Scenes/config_chess.txt
    # python load_colmap.py --config ../script/config/7Scenes/config_heads.txt
    ...
    from utils.utils import compute_ATE_error
