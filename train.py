...
def render_test(args, train_dl, val_dl, hwf, start, model, device, render_kwargs_test):
    ...
    if args.render_video_train:
        ...
        with torch.no_grad():
            rgbs, disps = render_path(poses_train.to(device), hwf, args.chunk, render_kwargs_test, gt_imgs=images_train,
                                      savedir=None)
    ...
    if args.render_video_test:
        ...
        with torch.no_grad():
            rgbs, disps = render_path(poses_val.to(device), hwf, args.chunk, render_kwargs_test, gt_imgs=images_val,
                                      savedir=None)
...
def train():
    ...
    # start training
    if args.dataset_type == '7Scenes':
        train_feature_matching(args, model, feat_model, optimizer, i_split, hwf, near, far, device, early_stopping,
